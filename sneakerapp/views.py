...
from sneakerapp.models import Product,Cart,Order
from django.db.models import Q 
from sneakerapp.forms import EmpForm,ProductModelForm,UserForm
...
def index(request):
   # data=Product.objects.all()#select * from ecomapp_products;
    # print(data)
    data=Product.objects.filter(status=1)#fetch only active products
    content={}
    content['products']=data
...
def addition(request,x,y):
    z=int(x)+int(y)
    print("Addition is:",z)
    return HttpResponse("Addition is:"+z)
...
def product_list(request):
    ...
    context['plist']=[
        ...
        {'name':'Iphone','pimage':'image of product','price':30000,'desc':'Product Description'}
    ]   
...
#delete from cart
def dc(request,cid):
    d=Cart.objects.filter(id=cid)
    d.delete()
    return redirect("/viewcart")

#delete from order
def dol(request,did):
    l=Order.objects.filter(id=did)
    l.delete()
...
def changeqtys(request,pid,g):
    content={}
    o=Order.objects.filter(pid=pid)
    if g=='1':
        x=o[0].qty+1
    else:
        x=o[0].qty-1
...
def makepayment(request):
    userid=request.user.id
    client=razorpay.Client(auth=("rzp_test_2oaoXpLQuR4Gv2","9tTstu3mNUBeCu3BBRoKrlBP"))
